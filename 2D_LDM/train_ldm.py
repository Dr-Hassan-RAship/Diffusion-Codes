...
def setup_environment(seed: int, snapshot_dir: str):
    ...
    output_file = "model_params.txt" # Define the output text file pathweights_only=True
...
def train_one_epoch(model, dae_image, dae_mask, train_loader, optimizer, inferer, scaler, scheduler, device, epoch, writer):
    """Train the LDM for one epoch."""
    epoch_loss   = 0.0
    # progress_bar = tqdm(enumerate(train_loader), total=len(train_loader), ncols=110)
    # progress_bar.set_description(f"Epoch {epoch + 1}/{N_EPOCHS}")
    model.train(); dae_image.eval(); dae_mask.eval()

    for step, batch in enumerate(train_loader):
        clean_image, clean_mask = batch["clean_image"].to(device), batch["clean_mask"].to(device)
...
def validate_one_epoch(model, dae_image, dae_mask, val_loader, inferer, scheduler, device, epoch, writer):
    """Validate the LDM for one epoch."""
    val_loss = 0.0
    model.eval(); dae_image.eval(); dae_mask.eval()
...
# ------------------------------------------------------------------------------#
def main():
    snapshot_dir = LDM_SNAPSHOT_DIR; check_or_create_folder(snapshot_dir)
    device       = setup_environment(SEED, snapshot_dir)
    models_dir   = os.path.join(snapshot_dir, "models"); check_or_create_folder(models_dir)
    ...
    train_loader         = get_dataloaders(BASE_DIR,
                                           split_ratio = SPLIT_RATIOS,
                                           split       = "train",
                                           trainsize   = TRAINSIZE,
                                           batch_size  = BATCH_SIZE,
                                           format      = FORMAT)
    val_loader           = get_dataloaders(BASE_DIR,
                                           split_ratio = SPLIT_RATIOS,
                                           split       = "val",
                                           trainsize   = TRAINSIZE,
                                           batch_size  = BATCH_SIZE,
                                           format      = FORMAT)
    tup_image, tup_mask  = load_autoencoder(device, train_loader, image = True, mask = True, epoch_image = 250, epoch_mask = 200)
    unet                 = DiffusionModelUNet(**MODEL_PARAMS)

    if RESUME_PATH:
        epoch_add = int(RESUME_PATH.split('_')[-1][:-4])
        writer = SummaryWriter(f"{snapshot_dir}/log")
        unet.load_state_dict(torch.load(RESUME_PATH, map_location=device, weights_only=True))
        unet.to(device)

    scheduler            = (DDIMScheduler(num_train_timesteps = NUM_TRAIN_TIMESTEPS, schedule = NOISE_SCHEDULER)
                            if SCHEDULER == 'DDIM'
                            else DDPMScheduler(num_train_timesteps = NUM_TRAIN_TIMESTEPS, schedule = NOISE_SCHEDULER))
    ...
    optimizer            = torch.optim.AdamW(unet.parameters(), lr = LR, betas = (0.9, 0.999), weight_decay = 0.0001)
    ...
    for epoch in range(N_EPOCHS): # Training Loop
        ...
        train_loss = train_one_epoch(unet, dae_image, dae_mask, train_loader, optimizer, inferer, scaler, scheduler, device, epoch, writer)
        ...
        if (epoch + 1) % VAL_INTERVAL == 0:
            val_loss = validate_one_epoch(unet, dae_image, dae_mask, val_loader, inferer, scheduler, device, epoch, writer)
