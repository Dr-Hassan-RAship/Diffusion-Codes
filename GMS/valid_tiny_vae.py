...
def save_binary_and_logits(
    x_logits, x_binary, name, save_seg_img_path, save_seg_logits_path, IMG_FORMAT=".png"
):
...
def load_img(path, img_size=224, dtype_resize=np.float32):
    ...
    image = (
        Image.open(path)
        .convert("L")
        .resize((img_size, img_size), resample=Image.NEAREST)
    )
...
def run_validator() -> None:
    ...
    if configs["vae_model"] == "tiny_vae":
        ...
        tiny_vae = get_tiny_autoencoder(
            train=False, residual_autoencoding=False
        )  # for the segmentation latent and decoding at the end.
        vae_model = get_lite_vae(
            model_version=configs["vae_model"], train=False, freeze=True
        )

    scale_factor = 1.0  # default

    if vae_train and skff_module is None:
        mapping_model, vae_model, _ = load_checkpoint(
            mapping_model,
            configs["model_weight"],
            vae_model=vae_model,
            vae_model_load=vae_train,
        )
    elif vae_train and skff_module is not None:
        mapping_model, vae_model, skff_module = load_checkpoint(
            mapping_model,
            configs["model_weight"],
            vae_model=vae_model,
            vae_model_load=vae_train,
            skff_model=skff_module,
            skff_model_load=True,
        )
    ...
    for batch_data in tqdm(valid_dataloader, desc="Valid: "):
        ...
        with torch.no_grad():
            ...
            save_binary_and_logits(
                x_logits, x_binary, name, save_seg_img_path, save_seg_logits_path
            )
    ...
    #     print(f'testing case_name: {case_name}')

    for case_name in tqdm(name_list):
        # print(f'true_path: {true_path}, pred_binary_path: {pred_binary_path}, pred_logits_path: {pred_logits_path}, case_name: {case_name}')
        seg_binary = load_img(
            os.path.join(pred_binary_path,  case_name + "_binary" + IMG_FORMAT)
        )
        seg_logits = load_img(
    ...
    ssim_region_list.extend(
        [np.mean(ssim_region_list), np.std(ssim_region_list, ddof=1)]
    )
    ssim_object_list.extend(
        [np.mean(ssim_object_list), np.std(ssim_object_list, ddof=1)]
    )
    ssim_combined_list.extend(
        [np.mean(ssim_combined_list), np.std(ssim_combined_list, ddof=1)]
    )
    ...
    logging.info(
        "DSC: {:.4f}, IOU: {:.4f}, HD95: {:.2f}".format(
            dsc_list[-2], iou_list[-2], hd95_list[-2]
        )
    )
