...
class Image_Dataset(Dataset):
        ...
        if self.stage == 'train':
            if self.dino_patch == 392:
                transforms_img = A.Compose([
                    ...
                    A.Resize(self.dino_patch, self.dino_patch),
                    ToTensorV2(),
                ])
                ...
                return transforms_img, transforms_mask
            else:
    ...
    def __getitem__(self, index):
        ...
        dino_patch, aug_img, aug_seg = None, None, None
        if self.dino_patch == 392:
            transforms_img, transforms_mask = self.get_transforms()
            ...
            augmented = transforms(image=img_data, mask=seg_data)
            aug_img = augmented['image']
            aug_seg = augmented['mask'].permute(2, 0, 1)
        
        # Add channel dimension to mask if it's missing
        # if aug_seg.ndim == 2:
        #     aug_seg = aug_seg.unsqueeze(0)

        if dino_patch is None:
            return {
                'name': name,
                'img': aug_img,
                'seg': aug_seg,
