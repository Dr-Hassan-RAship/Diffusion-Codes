...
class LiteVAEEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.wavelet_fn = HaarTransform(levels = self.levels)

        # Shared UNets (1 per level)
        self.feature_extractor_L1 = LiteVAEUNetBlock(in_channels = 4, out_channels = 4) # replacing in_channels with #sub-bands
        self.feature_extractor_L2 = LiteVAEUNetBlock(in_channels = 4, out_channels = 4)
        self.feature_extractor_L3 = LiteVAEUNetBlock(in_channels = 4, out_channels = 4)
        ...
        self.feature_aggregator = LiteVAEUNetBlock(
            in_channels = aggregated_channels, out_channels = out_channels,
        )
