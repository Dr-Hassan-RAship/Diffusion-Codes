...
class HPPF(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels, int(in_channels / 16), 1, 1), nn.ReLU(inplace=True))
        self.conv2 = nn.Sequential(nn.Conv2d(in_channels, int(in_channels / 64), 1, 1), nn.ReLU(inplace=True))
        self.avg   = nn.AdaptiveAvgPool2d(1)
        self.max1  = nn.AdaptiveMaxPool2d(4)
        self.max2  = nn.AdaptiveMaxPool2d(8)
        self.mlp   = nn.Sequential(
            nn.Conv2d(in_channels, int(in_channels / 8), kernel_size=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(int(in_channels / 8), in_channels, kernel_size=1),
            nn.Sigmoid())
        self.feat_conv = nn.Sequential(nn.Conv2d(in_channels, int(in_channels / 3), 3, 1, 1),
                                       nn.BatchNorm2d(int(in_channels / 3)),
                                       nn.ReLU(inplace=True))
...
class CBAM(nn.Module):
    ...
    def __init__(self, channel, reduction=16):
        ...
        self.mlp = nn.Sequential(
            nn.Conv2d(channel, int(channel / reduction), kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(int(channel / reduction), channel, kernel_size=1, bias=False))
