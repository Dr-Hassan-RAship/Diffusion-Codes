...
def run_trainer() -> None:
    ...
    for epoch in range(1, configs["epochs"] + 1):
        ...
        for batch_data in tqdm(train_dataloader, desc="Train: "):
            ...
            img_latent_mean, img_latent_logvar = get_vae_encoding_mu_and_sigma(
                vae_model.encode(get_cuda(img_rgb)), scale_factor
            )
            seg_latent_mean, seg_latent_logvar = get_vae_encoding_mu_and_sigma(
                vae_model.encode(get_cuda(seg_rgb)), scale_factor
            )
